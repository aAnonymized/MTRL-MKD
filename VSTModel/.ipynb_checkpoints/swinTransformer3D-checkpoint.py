...
class I3DHead(nn.Module):
    ...
    def forward(self, feature_map, Attn_states_out):
        # [N, in_channels, 4, 7, 7]
        ...
        # [N, num_classes]
        return cls_score, x, Attn_states_out
...
class SwinTransformer3D(nn.Module):
    ...
    Args:
        ...
        attn_drop_rate (float): Attention dropout rate. Default: 0.pretrained2d
    ...
    def __init__(self,
        ...
        self.head = I3DHead(num_classes=num_class)
        self._freeze_stages()
    ...
    def forward(self, x):
        ...
        for layer in self.layers:
            ...
            # print(f'x : {x.shape}')
            Attn_states_out.append(x)

        x = rearrange(x, 'n c d h w -> n d h w c')
        x = self.norm(x)
        x = rearrange(x, 'n d h w c -> n c d h w')

        return self.head(x, Attn_states_out)
